...
describe("GET All Templates", () => {
    test("Success Get All Templates - should return status 200", async (done) => {
        try {
            ...
            expect(body).toEqual(
                expect.arrayContaining([expect.objectContaining(inputTemplate)])
            );
            done();
        } catch (error) {
            done(error);
